...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      ...
      }, "\u5E94\u7528\u7A0B\u5E8F\u9519\u8BEF")), /*#__PURE__*/React.createElement("p", {
        className: "text-stone-600 mb-4"
      }, "\u53D1\u751F\u4E25\u91CD\u7684\u521D\u59CB\u5316\u9519\u8BEF\u3002\u8BF7\u91CD\u7F6E\u5E94\u7528\u7A0B\u5E8F\u4EE5\u6E05\u9664\u5148\u524D\u7248\u672C\u7684\u7F13\u5B58\u3002"), /*#__PURE__*/React.createElement("div", {
...
const NUTRIENTS = [{
  ...
  enzymes: [{
    ...
    zh_name: '硝酸还原酶',
    zh_func: '氮气同化的守门员 (NO3 → NO2)'
  }, {
    name: 'RuBisCO',
    func: 'Photosynthesis Engine (Fixes CO2)',
    zh_name: 'RuBisCO (核酮糖-1,5-二磷酸羧化酶)',
    zh_func: '光合作用引擎 (固定 CO2)'
  }, {
    name: 'PAL',
  ...
  enzymes: [{
    ...
    zh_name: '亚硝酸还原酶',
    zh_func: '蛋白质构建 (NO2 → NH4)'
  }, {
    name: 'APX',
    func: 'Chloroplast Antioxidant',
    zh_name: 'APX (抗坏血酸过氧化物酶)',
    zh_func: '叶绿体抗氧化剂'
  }],
  optimal_levels: {
    soil: {
...
const analyzeSoilImpact = (nutrientId, soil) => {
  ...
  if (soil.ph < 5.5) {
    if (nutrientId === 'N') {
      ...
      impacts.push({
        level: 'high',
        msg: '酸性锁定：pH < 5.5 时，大量元素和钼的有效性显著下降。铝毒性可能会干扰。'
      });
    }
    if (['Fe', 'Mn', 'Zn', 'Cu', 'B'].includes(nutrientId)) {
      impacts.push({
        level: 'low',
        msg: '溶解度增加：微量元素在酸性中极易溶解。注意毒性（尤其是锰）。'
      });
    }
    if (nutrientId === 'P') {
      impacts.push({
        level: 'high',
        msg: '铝/铁固定：极高风险。磷与铝/铁结合成不溶性固体。'
      });
    }
  } else if (soil.ph > 7.5) {
    if (['Fe', 'Mn', 'Zn', 'Cu', 'B'].includes(nutrientId)) {
      impacts.push({
        level: 'high',
        msg: '碱性固定：pH > 7.5 时，微量元素（除钼外）沉淀。必须使用螯合物或叶面喷施。'
      });
    }
    if (nutrientId === 'P') {
      impacts.push({
        level: 'med',
        msg: '钙固定：磷与钙结合形成不溶性的磷酸钙。'
      });
    }
    if (nutrientId === 'Mo') {
      impacts.push({
  ...
  if (soil.texture === 'sand') {
    if (['N', 'K', 'Mg', 'S', 'B'].includes(nutrientId)) {
      impacts.push({
        level: 'high',
        msg: '流失风险：沙土保肥力 (CEC) 低。该元素容易流失。需“少食多餐”。'
      });
    }
  ...
  if (soil.om === 'low') {
    if (['N', 'S'].includes(nutrientId)) {
      impacts.push({
        level: 'med',
        msg: '储备低：土壤缺乏有机氮/硫库。高度依赖肥料。'
      });
    }
    if (['Fe', 'Zn', 'Cu', 'Mn'].includes(nutrientId)) {
      impacts.push({
        level: 'med',
        msg: '无天然螯合：低有机质意味着微量元素容易被土壤矿物固定。'
      });
    }
  ...
  if (soil.cation === 'hi_ca') {
    if (nutrientId === 'Mg') {
      ...
      impacts.push({
        level: 'high',
        msg: '石灰诱发失绿：高钙通常意味着高 pH，导致微量元素沉淀。'
      });
    }
...
const getSmartRecommendation = inputs => {
  ...
  if (stage === 'seedling') {
    ...
    if (crop === 'durian') {
      ...
      switch (variety) {
        case 'gp':
          daaText = "金凤 (D198) 自然落果大约在 90–100 DAA。";
          break;
        case 'mk':
          daaText = "猫山王 (D197) 自然落果大约在 95–105 DAA。";
          break;
        case 'rp':
          daaText = "红虾 (D175) 自然落果大约在 100–105 DAA。";
          break;
        case 'd101':
          daaText = "红肉 (D101) 自然落果大约在 100–105 DAA。";
          break;
        case 'tekka':
          daaText = "竹脚 (D160) 自然落果大约在 100–105 DAA。";
          break;
        case 'd24':
          daaText = "苏丹王 (D24) 自然落果大约在 105–115 DAA。";
          break;
        case 'tk':
          daaText = "松鼠王 (D214) 自然落果大约在 105–110 DAA。";
          break;
        case 'bt':
          daaText = "黑刺 (D200) 自然落果大约在 110–120 DAA。";
          break;
        case 'ioi':
          daaText = "IOI (D168) 自然落果大约在 110–115 DAA。";
          break;
        case 'mt':
          daaText = "金枕头 (D159) 自然落果大约在 120–130 DAA。";
          break;
        default:
          daaText = "一般甘榜克隆品种通常在 90–110 DAA 左右落果。";
          break;
      }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `text-sm md:text-base font-black leading-tight text-right md:text-center ${meta.leach_desc.includes('高') ? 'text-red-600' : 'text-stone-900'}`
      ...
      }, meta.compete === '无' ? t('lbl_none') : meta.compete))))), /*#__PURE__*/React.createElement("div", {
...
const SmartAdvisorPanel = ({
  ...
  }, "\u9884\u8BA1\u6536\u83B7\u7A97\u53E3\u671F"), draftState.variety === 'gp' && '金凤 (D198) 自然落果大约在 90–100 DAA。', draftState.variety === 'mk' && '猫山王 (D197) 自然落果大约在 95–105 DAA。', draftState.variety === 'rp' && '红虾 (D175) 自然落果大约在 100–105 DAA。', draftState.variety === 'd101' && '红肉 (D101) 自然落果大约在 100–105 DAA。', draftState.variety === 'tekka' && '竹脚 (D160) 自然落果大约在 100–105 DAA。', draftState.variety === 'd24' && '苏丹王 (D24) 自然落果大约在 105–115 DAA。', draftState.variety === 'tk' && '松鼠王 (D214) 自然落果大约在 105–110 DAA。', draftState.variety === 'bt' && '黑刺 (D200) 自然落果大约在 110–120 DAA。', draftState.variety === 'ioi' && 'IOI (D168) 自然落果大约在 110–115 DAA。', draftState.variety === 'mt' && '金枕头 (D159) 自然落果大约在 120–130 DAA。', draftState.variety === 'generic' && '一般甘榜克隆品种通常在 90–110 DAA 左右落果。'))), /*#__PURE__*/React.createElement("div", {
    className: "space-y-1 md:col-span-2"
  }, /*#__PURE__*/React.createElement("label", {
    className: "text-xs font-bold text-stone-500 uppercase tracking-wide"
...
const App = () => {
  ...
  return /*#__PURE__*/React.createElement(ErrorBoundary, null, /*#__PURE__*/React.createElement("div", {
  ...
  }, t('msg_immobile_advice'))), TRANSLATIONS.zh && TRANSLATIONS.zh['info_' + selectedNutrient.id + '_bio'] && /*#__PURE__*/React.createElement("div", {
  ...
  }).join(', ') || t('lbl_none')), TRANSLATIONS.zh && TRANSLATIONS.zh['info_' + selectedNutrient.id + '_antag'] && /*#__PURE__*/React.createElement("div", {
    ...
    const translatedName = TRANSLATIONS.zh['elem_' + rawName] ? TRANSLATIONS.zh['elem_' + rawName] : rawName;
    const displayVal = t(item.val);
    return /*#__PURE__*/React.createElement("div", {
      key: idx,
